...
- `npx wrangler secret put <NAME>` — set secrets listed in `wrangler.jsonc` (`ADMIN_PIN`, `TWILIO_*`, optional `MAX_ALERTS_PER_MONTH`, optional `TWILIO_ANNOUNCE_TEMPLATE_SID`)
...
- Seed/manage family members directly via `npx wrangler kv key put --binding=FAMILY "+E164" '{"name":"...","is_admin":true,"joined_at":0,"opted_in":true}'`
...
- Location shares (Latitude/Longitude present) → `handleLocationShare` (auto-marks as HELP if no prior check-in).
- Free-text from admins → `handleAdminCommand` (`src/commands.ts`). Includes `announce <pin> <message>` for broadcasting announcements and optional `[message]` on `alert`/`test` commands.
- Free-text from regular members → `handleFamilyMessage` (matches `safe` / `help` patterns during an active incident).
- Unknown senders → `unknownSenderReply` + fire-and-forget admin notification.
...
3. The `alarm()` callback marks `fired = true`, fans out the approved Twilio template (`TWILIO_TEMPLATE_SID`) to every opted-in member using staggered sends (500ms delay between each), and DMs the admin a delivery summary.
...
5. `recordMessage` captures follow-up text from HELP responders.
6. `recordLocation` attaches location data to a response.
7. `loadActiveIncident()` is the read-side helper used by both `handleFamilyMessage` and `statusCommand`. It loads the open-incident pointer, fetches the DO state, and treats the incident as closed (clearing the pointer as a side effect) if it's missing, cancelled, or older than `INCIDENT_ACTIVE_WINDOW_MS` (2 hours).

**Status board fanout** is cost-controlled by the WhatsApp 24-hour service window. After every check-in, only family members whose window is provably open get the updated board: anyone who has already responded to *this* incident, plus the admin who triggered it. Members who haven't tapped yet see the board only when they themselves tap. `sendBoard` (`src/statusBoard.ts`) sends a fresh message each time.

**State storage layout**:
- `FAMILY` KV — per-phone roster entries (`{name, is_admin, joined_at, opted_in}`), key is the E.164 phone with `+`. Also stores `_welcome_template_sid` (Twilio Content Template SID for the welcome message, created via `welcome` admin command).
- `RATE_LIMIT` KV — 5-minute inter-alert lockout (overridable with `alert <pin> force`). Also stores announcement idempotency cache and monthly alert counts.
...
- `Incident` DO storage — the only strongly-consistent store. Holds full `IncidentState` (responses, processed webhook sids, alarm flags).
